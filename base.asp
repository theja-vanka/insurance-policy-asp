...
costA(RID, COST) :- referee(RID, e, _, _, _), COST=#sum{PAYMENT, CID: case(CID, _, _, _, _, PAYMENT), assign(CID, RID)}.
...
avgcost(COST) :- uniqueErefree(ECOUNT), SCOST=#sum{ICOST, RID: overallcost(RID, ICOST)}, COST=SCOST/ECOUNT, ECOUNT>0. 
costB(RID, DIV) :- overallcost(RID, OCOST), avgcost(COST), DIV=|COST-OCOST|.
costB(RID, DIV) :- not avgcost(COST), DIV=0.
...
avgtime(TIME) :- uniquerefree(COUNT), STIME=#sum{ITIME, RID: overalltime(RID, ITIME)}, TIME=STIME/COUNT.
...
costD(RID, COST) :- referee(RID, _, _, _, _), COST=#sum{3-PREFCOST, CID: case(CID, CTYPE, _, _, _, _), prefType(RID, CTYPE, PREFCOST), assign(CID, RID)}.
...
costE(RID, COST) :- referee(RID, _, _, _, _), COST=#sum{3-PREFCOST, CID: case(CID, _, _, _, POST, _), prefRegion(RID, POST, PREFCOST), assign(CID, RID)}.

% The overall costs to be minimized are given as follows:
% c = 16⋅cA + 7⋅cB + 9⋅cC + 34⋅cD + 34⋅cE
costMin(RID, COST) :- costA(RID,ACOST), costB(RID, BCOST), costC(RID, CCOST), costD(RID, DCOST), costE(RID, ECOST), COST=16*ACOST+7*BCOST+9*CCOST+34*DCOST+34*ECOST.
#minimize{COST: costMin(_,COST)}.
